...
clc
SNR_dB =10;%  SNR(dB)
N=1000; % dimension of transmitted signal x
M=4*N ; % dimension of observation y
lamda=0.5; %sparasity
tr=100;% number of realizations
t_max=50; %number of iteration
L=1; % number of decentralized clusters
...
%% Run deGEC-SR (L > 1) or GEC-SR (L=1)
    if L ~=1
        [MSE_mtx_GLM(:,jj),x_est3,x_hat3]...
           = DeGEC_SR(x,initial,N,M,t_max, svd_eig, gOut, gxIn,A,L,U,V,D); % run GLM 
    else
        [MSE_mtx_GLM(:,jj),x_est3,x_hat3]...
           = GEC_SR(x,initial,N,M,t_max, svd_eig, gOut, gxIn,A,L,u,v); % run GLM
    end
